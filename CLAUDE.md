...
- Always run the project after code changes using the godot-mcp MCP tools and check `get_debug_output` for errors before reporting done.- **Run the full GdUnit4 test suite before committing** and verify zero failures. Use: `Start-Process -FilePath $godot -ArgumentList '--path','.','-s','res://addons/gdUnit4/bin/GdUnitCmdTool.gd','--add','res://test/' -PassThru -Wait` then check `reports/report_*/results.xml` for 0 failures. **Do NOT use `--headless`** — GdUnit4 does not support headless mode.
